...
public class MemberDao {
	...
	{
		int result = 0;
		String query = "update member set USERPWD = ? , USERNAME = ? , PHONE = ? , ADDRESS = ? , PWDQUESTION = ? , PWDANSWER = ? , MARRIAGE = ? , HOBBY = ? , ETC = ?" +
		" WHERE USERID = ?";
		Connection conn = getConnection();
		PreparedStatement pstmt = null;
		...
		try {
			...
			pstmt.setString(5, m.getPasswordQuestion());
			pstmt.setString(6, m.getPasswordAnswer());
			pstmt.setString(7, m.getMarriage());
